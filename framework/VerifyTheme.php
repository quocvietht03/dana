 ...
 */
define("ITEM_ID","21244862"); // Replace it by current item id
...
class VerifyTheme {
    ...
    // add style admin
      ...
      if(!$this->isInstallationLegit){
        $setting_page = admin_url('options-general.php?page=verifytheme_settings');
        $message = esc_html__( "Important notice: In order to receive all benefits of our theme, you need to activate your copy of the theme. \nBy activating the theme license you will unlock premium options - import demo data, install, update plugins and official support. Please visit Envato Settings page to activate your copy of the theme", 'dana' );
    ...
    public function verifytheme_settings_page(){
        ...
        $other_attributes = '';
        $register_button_text = __( 'Register your theme', 'dana' );
        if ( $toolkitData && $installationLegit ){
          $other_attributes = 'disabled';
          $register_button_text = __( 'Activated on this domain', 'dana' );
        ...
        <div class="wrap verifytheme_wrap">
            ...
            <form style="display: none" id="change_license_form" method="POST">
              <button id="change_license_btn" type="submit" class="button button-primary" name="change_license"><?php echo esc_html__( 'Deregister your product', 'dana' ); ?></button>
            </form>
        </div>
        <?php
    }
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if(!empty($message)):
          ...
          if ( ! $already_in_use ):
            ...
            $message .= sprintf(wp_kses( __( 'This product is in use on another domain: <span>%s</span><br />', 'dana' ), array( 'span' => array(), 'br' => array() ) ), $connected_domain );
            $message .= sprintf(esc_html__('Are you using this theme for a new site? Please purchase a %s ', 'dana' ), '<a tabindex="-1" href="' . esc_url( 'http://themeforest.net/cart/add_items?ref=bearsthemes&item_ids=' ) .ITEM_ID.'" target="_blank">'.esc_html__('new license','verifytheme').'</a>');
            if(!$register_error):
              add_settings_error(
                  'verifytheme_settings',
                  esc_attr( 'settings_updated' ),
