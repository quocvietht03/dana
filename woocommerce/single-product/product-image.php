...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
	<figure class="woocommerce-product-gallery__wrapper">
		<?php
		wp_enqueue_script('dana-zoom-master');

		$attachment_ids = $product->get_gallery_image_ids();

		if ( sizeof($attachment_ids) > 1 && has_post_thumbnail() ) {
			wp_enqueue_script('dana-slick-slider');
			wp_enqueue_style('dana-slick-slider');
			...
			if ( has_post_thumbnail() ) {
				...
				$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'dana' ) );
				$html .= '</div>';
			}
